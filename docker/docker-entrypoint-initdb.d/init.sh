...
sed -ri "s/#(wal_level) = .*/\1 = 'minimal'/" "${PGDATA}/postgresql.conf"
sed -ri "s/#(max_worker_processes) = .*/\1 = 10/" "${PGDATA}/postgresql.conf"
sed -ri "s/#(max_replication_slots) = .*/\1 = 20/" "${PGDATA}/postgresql.conf"
sed -ri "s/#(max_wal_senders) = .*/\0 = ${PG_MAX_WAL_SENDERS:-10}/" "${PGDATA}/postgresql.conf"
...
sed -ri "s/#(work_mem) = .*/\1 = ${PG_WORK_MEM:-4MB}/" "${PGDATA}/postgresql.conf"
sed -ri "s/#(checkpoint_warning) = .*/\1 = ${PG_CHEKPOINT_WARNING:-0}/" "${PGDATA}/postgresql.conf"
sed -ri "s/#(log_checkpoints) = .*/\1 = ${PG_LOG_CHECKPOINTS:-off}/" "${PGDATA}/postgresql.conf"
